...
?>
            ...
            <div class="lk_maincol">
                ...
                <div class="lk_block">
                    <main class="lk_content">
                        ...
                        <?= $form->field($cancel_model, 'id', ['options' => ['style' => 'display:none;']])->hiddenInput(); ?>

                        <h4 class="lk_step_title font20 mt20">Укажите причину отмены мероприятия</h4>
                        <?= $form->field($cancel_model, 'cancel_reason', ['options' => ['class' => 'ip_cell w100 mb50']])->textInput(); ?>

                        <h4 class="lk_step_title">Ознакомьтесь с текстом письма-уведомления об отмене мероприятия</h4>

                        <?= $form->field($cancel_model, 'cancel_letter', ['template' => '{input}{error}{hint}'])->widget(CKEditor::class, [
                        ...
                        <div class="ip_cell w100 flex justify-between buttons-wrapper mb0">
                            <button type="submit" class="button blue medium lk">Отменить мероприятие и отправить
                                рассылку
                            </button>
                        </div>
                        <?php ActiveForm::end(); ?>
    ...
    <div class="modal" id="cancel_event_modal">
        <div class="modal_content">
            ...
            <div class="success_box">
                <div class="modal_title">Отмена мероприятия</div>
                <p>Мероприятие отменено. Рассылка уведомлений участникам, купившим билеты, будет отправлена в ближайшее
                    время.</p>
    ...
    <div class="modal" id="fail_event_modal">
        <div class="modal_content">
            <a href="#" class="modal_close">x</a>
            <div class="success_box">
                <div class="modal_title">Ошибка при отмене мероприятия</div>
...
<?php
$url = Url::toRoute(['/pages/activities/sendcancelform']);
$js = <<<JS
    $('#event-cancel').on('beforeSubmit', function(event){
        var formData = new FormData($('#event-cancel')[0]);
        $.ajax({
            type: 'POST',
            url: '{$url}',
            contentType: false,
            ...
            success: function(data) {
                if (data.status == 'success') {
                    ...
                    $('#fail_event_modal .modal_title').html('Ошибка при отмене мероприятия');
    ...
    $('#event-cancel').on('submit', function(e){
        e.preventDefault();
        return false;
    });
JS;
