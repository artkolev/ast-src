<?php
/*
    @descr Архив заказов (Заказы эксперта, ЛК)

    @var $model Class app\modules\pages\models\OrdersIncomingArchive; текущая страница
    @var $items Array of app\modules\order\models\Order; массив заказов в архивных статусах, где пользователь либо заказчик, либо исполнитель
    @var $cur_status String текущий выбранный статус для показа (из get-параметров)

    @action pages/orders/archiveincoming
*/
...
$status_class = [
        Order::STATUS_DONE => ['color' => 'blue', 'icon' => 'published'],
        Order::STATUS_CLOSE => ['color' => 'darkgreen', 'icon' => 'end-order'],
];
            ...
            <div class="lk_maincol">
                ...
                <div class="lk_block desktop-visible">
                    <main class="lk_content lk_content-basic lk_switchers">
                        ...
                        <a href="<?= $model->getUrlPath(); ?>?status=done" class="button lk">Выполнен</a>
                        <a href="<?= $model->getUrlPath(); ?>?status=close" class="button lk darkgreen">Закрыт</a>
                    </main>
                </div>
                    ...
                    <?php foreach ($items as $item) { ?>
                        <div class="lk_order_item <?= $item->hasUnreadNotifications(Yii::$app->user->identity->userAR->id) ? 'has_notifications' : ''; ?>">
                            ...
                            <?php if ($item->status == Order::STATUS_NEW) {
                                $payment_url = (!empty($payment_page)) ? Url::toRoute([$payment_page->getUrlPath(), 'type' => 'order', 'id' => $item->id]) : false;
                                if ($payment_url) { ?>
                ...
                <?php } else { ?>
                    <div class="lk_block">
                        <main class="lk_content">
                            <?php
                            $types = ['done' => 'выполненных заказов', 'close' => 'закрытых заказов']; ?>
                            У вас нет <?= (isset($types[$cur_status]) ? $types[$cur_status] : 'завершенных заказов'); ?>
                            .
                        </main>
                    </div>
                <?php } ?>
