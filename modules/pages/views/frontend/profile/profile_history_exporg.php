            ...
            <div class="lk_maincol">
                <div class="lk_block">
                    ...
                    <main class="lk_content">
                        ...
                        <?= $form->field($modelform, 'products')->textInput(['placeholder' => "Направления деятельности"]); ?>

                        <?= $form->field($modelform, 'about_myself', ['template' => '{label}<div class="flex">{input}' . $modelform->getQuestion('about_myself') . '</div>{error}{hint}'])->textArea(['placeholder' => "Например, специалист по личностному развитию, доктор экономических наук"]); ?>
                        <?= $form->field($modelform, 'history')->textArea(['placeholder' => "Подробно об организации"]); ?>
                        <?= $form->field($modelform, 'video')->input('url', ['placeholder' => "Ссылка на видеопрезентацию"]); ?>
                        <?= $form->field($modelform, 'description')->textArea(['placeholder' => "Дополнительная информация"]); ?>
