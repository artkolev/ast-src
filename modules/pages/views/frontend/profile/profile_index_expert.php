            ...
            <div class="lk_maincol">
                <div class="lk_block">
                    <header class="lk_block_header">
                        <h1 class="lk_block_title-big">
                            <?= $model->getNameForView(); ?>
                        </h1>
                        <?= $model->content; ?>
                        <?= $model->expert_content; ?>
                        <?php
                        // TODO: Жду ответа Нино
                        // для ВЭС ранее выводился вот этот текст, если от него избавятся - нужно удалить поле в модели. Если нет - то написать здесь условие, когда выводить какой текст.
                        // = $model->academ_content;?>
                    </header>
                    <main class="lk_content">
                        ...
                        <?php if (!empty($modelform->getQuestion('surname'))) {
                            echo $form->field($modelform, 'surname', ['options' => ['class' => 'ip_cell w100 lock-on'], 'template' => '{label}<div class="flex">{input}' . $modelform->getQuestion('surname') . '<div class="pencil-field field-off"></div></div>{error}{hint}'])->textInput(['placeholder' => "Фамилия*"]);
                        } else {
                            echo $form->field($modelform, 'surname', ['options' => ['class' => 'ip_cell w100 lock-on'], 'template' => '{label}{input}{error}<div class="pencil-field field-off"></div>{hint}'])->textInput(['placeholder' => "Фамилия*"]);
                        } ?>
                        <?php if (!empty($modelform->getQuestion('name'))) {
                            echo $form->field($modelform, 'name', ['options' => ['class' => 'ip_cell w100 lock-on'], 'template' => '{label}<div class="flex">{input}' . $modelform->getQuestion('name') . '<div class="pencil-field field-off"></div></div>{error}{hint}'])->textInput(['placeholder' => "Имя*"]);
                        } else {
                            echo $form->field($modelform, 'name', ['options' => ['class' => 'ip_cell w100 lock-on'], 'template' => '{label}{input}{error}<div class="pencil-field field-off"></div>{hint}'])->textInput(['placeholder' => "Имя*"]);
                        } ?>
                        <?php if (!empty($modelform->getQuestion('patronymic'))) {
                            echo $form->field($modelform, 'patronymic', ['options' => ['class' => 'ip_cell w100 lock-on'], 'template' => '{label}<div class="flex">{input}' . $modelform->getQuestion('patronymic') . '<div class="pencil-field field-off"></div></div>{error}{hint}'])->textInput(['placeholder' => "Отчество"]);
                        } else {
                            echo $form->field($modelform, 'patronymic', ['options' => ['class' => 'ip_cell w100 lock-on'], 'template' => '{label}{input}{error}<div class="pencil-field field-off"></div>{hint}'])->textInput(['placeholder' => "Отчество"]);
                        } ?>
                        ...
                        <div class="ip_cell">
                            ...
                            <div class="time_box">
                                <?= $form->field($modelform, 'comfy_time_from', ['options' => ['class' => 'ip_cell ipс_short'], 'template' => '<div class="flex">{input}' . $modelform->getQuestion('comfy_time_from') . '</div>{error}{hint}'])->textInput(['placeholder' => "12:00", 'class' => 'input_text ip_short maskTime']); ?>
                                <span>&mdash;</span>
                                <?= $form->field($modelform, 'comfy_time_to', ['options' => ['class' => 'ip_cell ipс_short'], 'template' => '<div class="flex">{input}' . $modelform->getQuestion('comfy_time_to') . '</div>{error}{hint}'])->textInput(['placeholder' => "12:00", 'class' => 'input_text ip_short maskTime']); ?>
                            </div>
                        </div>
                        <?= $form->field($modelform, 'assist_name', ['options' => ['class' => 'ip_cell w100 lock-on'], 'template' => '{label}{input}{error}<div class="pencil-field field-off"></div>{hint}'])->textInput(['placeholder' => "ФИО ассистента"]); ?>
                        <?= $form->field($modelform, 'assist_phone', ['options' => ['class' => 'ip_cell w100 lock-on'], 'template' => '{label}{input}{error}<div class="pencil-field field-off"></div>{hint}'])->input('tel', ['placeholder' => "+7 (000) 000-00-00", 'class' => 'input_text phone-mask']); ?>
                        <?= $form->field($modelform, 'assist_email', ['options' => ['class' => 'ip_cell w100 lock-on'], 'template' => '{label}{input}{error}<div class="pencil-field field-off"></div>{hint}'])->input('email', ['placeholder' => "Email ассистента"]); ?>
    ...
    <div class="modal" id="fail_profile_modal">
        <div class="modal_content">
            ...
            <div class="success_box">
                ...
                <p><?= \app\helpers\MainHelper::getHelpText(); ?>
                </p>
