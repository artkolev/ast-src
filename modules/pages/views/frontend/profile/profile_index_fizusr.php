...
use app\modules\pages\models\ProfilePretendent;
use yii\helpers\Url;
...
if ($user->pretendent) {
    ...
    $profile_pretendent = ProfilePretendent::find()->where(['model' => ProfilePretendent::class, 'visible' => 1])->one();
    ...
    if ($norm_cafedra) {
        if ($profile_pretendent) {
            $anketa_button = $profile_pretendent->getUrlPath();
        }
    } else {
        if ($profile_pretendent and $profile_cafedra) {
            $anketa_button = $profile_cafedra->getUrlPath();
        }
    }
}
...
?>
    <main class="sec content_sec gray_bg">
        <div class="container wide lk-container">
            <div class="lk_maincol">
                <div class="lk_block">
                    <header class="lk_block_header">
                        ...
                        <?= $model->content; ?>
                        <?= $model->fizusr_content; ?>
                        <?php if ($anketa_button) { ?>
                            <a href="<?= $anketa_button; ?>" class="button-o lk_button_submit mt50">Заполнить анкету
                                Эксперта</a><br><br>
                        <?php } ?>
                    </header>
            ...
            <?= \app\modules\users\widgets\profile\FizmenuWidget::widget(); ?>
        </div>
    </main>
