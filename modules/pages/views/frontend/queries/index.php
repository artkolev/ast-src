<?php
/*
    @descr Текущие запросы (Запросы клиента, ЛК)

    @var $model Class app\modules\pages\models\QueriesList; текущая страница
    @var $items Array of app\modules\queries\models\Queries; массив запросов Клиента
    @var $cur_status String текущий выбранный статус для показа (из get-параметров)

    @action pages/queries/index
*/
...
$status_class = [
        ...
        Queries::STATUS_OFFER => ['color' => 'yellow', 'icon' => 'inwork'],
];
                ...
                <div class="lk_block desktop-visible">
                    <main class="lk_content lk_content-basic lk_switchers">
                        ...
                        <a href="<?= $model->getUrlPath(); ?>?status=agreement" class="button lk darkgreen">Обсуждение
                            условий</a>
                        <a href="<?= $model->getUrlPath(); ?>?status=offer" class="button lk yellow">Предложения</a>
                    </main>
                </div>
                    ...
                    <div class="lk_block_header">
                        <div class="lk_switchers-select-wrapper mb0">
                            ...
                            <select class="lk_switchers-select">
                                ...
                                <option value="<?= $model->getUrlPath(); ?>?status=offer" <?= $cur_status == 'offer' ? 'selected' : ''; ?>>
                                    Предложения
                                </option>
                            </select>
                        </div>
                ...
                <?php } else { ?>
                    <div class="lk_block">
                        <main class="lk_content">
                            <?php
                            $types = ['new' => 'новых запросов', 'agreement' => 'запросов на этапе обсуждения условий', 'offer' => 'запросов, ожидающих предложений']; ?>
