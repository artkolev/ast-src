...
$reg_page = PServiceRegOOO::find()->where(['model' => PServiceRegOOO::class, 'visible' => 1])->one();
?>
            ...
            <div class="lk_maincol">
                <div class="lk_block">
                    ...
                    <main class="lk_content">
                        <?= $model->content; ?>
                        <br>
                        ВНИМАНИЕ! Если Вы не являетесь генеральным директором юридического лица, то Вам необходимо
                        получить заверенное подписью генерального директора Соглашение об аналоге собственноручной
                        подписи и прикрепить скан к анкете. <?php if (!empty($reg_page->primer)) { ?><a
...
$js = <<<JS
    $('.open_wait').click(function(e){
        e.preventDefault();
        ym(67214377,'reachGoal','register_ooo');
        modalPos('#wait_modal');
        return false;
    });
